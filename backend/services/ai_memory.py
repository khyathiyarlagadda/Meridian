...
def initialize_or_load_memory(force_recompute: bool = False) -> List[Dict[str, Any]]:
    """
    Initializes AI memory entries backed by real dataset aggregations and merchant settings via Supabase cloud DB.
    """
    from supabase_db import get_ai_memories, save_ai_memory
    ...
    saved = get_ai_memories()
    if not force_recompute and saved and len(saved) > 0:
        for m in saved:
...
def record_memory_usage(memory_key: str):
    ...
    memories = get_ai_memories()
    for m in memories:
        if m["key"] == memory_key:
            m["applied_count"] = int(m.get("applied_count", 1)) + 1
